...
현재 구현은 `04 센서 경보` 학습지 보조 도구로 제한한다.
...
- 모음 페이지에서 아직 구현하지 않은 `01`, `02`, `03`, `05`, `06`, `07`, `08`, `09`, `10`, `11`은 준비 중 카드로 두고 링크하지 않는다.
...
- 외부 CDN은 쓰지 않는다. 현재 앱에는 MathJax가 필요하지 않다.
...
01 상품추천, 02 우주통신, 03 행사준비, 05 A/B실험, 06 불량품검사, 07 직렬병렬,
08 학교시설, 09 축제부스, 10 우천행사, 11 설문표본: 준비 중 카드, 링크 없음
```

## 5. 센서 경보 preset 데이터

센서 경보 기본값은 실제 학생용 학습지와 일치시킨다.
...
- 현재 앱에서는 학습지 값 중심으로 두고, 자유로운 모델 조작은 후속 확장으로 미뤄도 된다.

## 7. 두 센서 모두 경보 모델

Q5 보조 기능은 다음 구조만 보여주고, 최종값은 학생이 두 항을 입력했을 때만 비율로 계산한다.
...
- `100 × 0.95²`, `9,900 × 0.05²` 구조는 보여줄 수 있지만, 각 빈칸의 계산값은 미리 채우지 않는다.
...
- 분모 입력
- 입력값에 대한 비율과 백분율 계산
- Q5처럼 두 항을 입력하면 두 항의 합을 분모로 하는 비율 계산
- Q6처럼 기저율 변경 후 다시 계산한 두 경보 건수의 비율 계산
...
- 교사용 자료가 웹앱 폴더에 없다.
- 모음 페이지에서 센서 경보는 실제 링크, 나머지 예정 웹앱은 준비 중 카드다.
- 센서 경보 앱에서 센서 경보 preset이 활성화되어 있고 상품추천 preset은 준비 중 상태다.
- 기본값이 학습지와 일치한다.
- `P(경보 | 위험)`과 `P(위험 | 경보)`의 분모 차이가 화면에서 드러난다.
- Q3의 헛경보와 놓침 위치를 빈도표에서 강조할 수 있다.
- Q4 판단 메모는 체크리스트로만 제공하고 대응안을 고르지 않는다.
- Q5와 Q6은 학생이 계산한 항을 입력해야 비율을 표시한다.
- Q7과 Q8은 점검 질문만 제공하고 반박 문장이나 최종 결론을 쓰지 않는다.
...
2. `probability-statistics/statistical-decision-lab/index.html`에서 센서 경보 링크와 나머지 예정 웹앱 준비 중 카드가 보이는지 확인한다.
...
11. Q3 버튼이 빈도표의 헛경보와 놓침 위치만 강조하는지 확인한다.
12. Q5와 Q6 계산기는 입력 전 결과를 표시하지 않고 입력 후 비율만 표시하는지 확인한다.
13. Q7과 Q8 영역이 정답 문장이 아니라 점검 기준만 제공하는지 확인한다.
14. iPad A16 가로 `1180×820`에서 PDF와 도구가 함께 보이는지 확인한다.
15. iPad A16 세로 `820×1180`에서 `학습지 보기`와 `도구 보기`가 서로 전환되는지 확인한다.
16. 화면 어디에도 최종 대응안 추천이나 Q8 정답 문장이 없는지 확인한다.
17. 개발자 도구 콘솔에 앱 코드 오류가 없는지 확인한다.
