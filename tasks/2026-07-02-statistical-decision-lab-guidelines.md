...
- `probability-statistics/statistical-decision-lab/sensor-alarm-student.pdf`
...
- `04 센서 경보`는 활성화한다.
- `01 상품추천`은 버튼만 두고 실제 기능은 비활성화한다.
...
- PDF 임베드는 `<object data="./sensor-alarm-student.pdf" type="application/pdf">` 또는 동등한 정적 방식으로 구현한다.
...
계산 기능은 학생이 학습지에서 구한 값을 확인하는 용도로만 둔다.
...
- 공통 CSS `../../assets/common.css`를 연결한다.
...
- 교사용 자료가 웹앱 폴더에 없다.
- 센서 경보 preset이 활성화되어 있고 상품추천 preset은 준비 중 상태다.
- 기본값이 학습지와 일치한다.
...
1. `probability-statistics/statistical-decision-lab/index.html`을 브라우저에서 연다.
2. 학생용 PDF가 보이고 다운로드 링크가 `./sensor-alarm-student.pdf`를 가리키는지 확인한다.
3. 웹앱 폴더에 교사용 PDF, 교사용 Markdown, 예시답안 파일이 없는지 확인한다.
4. 센서 경보 preset이 활성화되어 있고 상품추천 preset이 준비 중 상태인지 확인한다.
5. 기본값이 `N = 10,000`, 기저율 `1%`, 민감도 `95%`, 오경보율 `5%`인지 확인한다.
6. 기본 자연빈도가 `95`, `5`, `495`, `9,405`로 계산되는지 확인한다.
7. `P(위험 | 경보)`가 `95 / 590 ≈ 16.1%`로 확인되는지 확인한다.
8. iPad A16 가로 `1180×820`에서 PDF와 도구가 함께 보이는지 확인한다.
9. iPad A16 세로 `820×1180`에서 `학습지 보기`와 `도구 보기`가 서로 전환되는지 확인한다.
10. 화면 어디에도 최종 대응안 추천이나 Q8 정답 문장이 없는지 확인한다.
11. 개발자 도구 콘솔에 앱 코드 오류가 없는지 확인한다.
12. `probability-statistics/index.html`의 카드가 `./statistical-decision-lab/`로 연결되는지 확인한다.
