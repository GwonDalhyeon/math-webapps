...
웹앱에 맞는 학습지 PDF를 만든 경우, 사용자가 별도로 요청하지 않아도 같은 폴더의 `index.html`에서 PDF를 다운로드할 수 있게 연결한다. Markdown 원본도 함께 제공할 수 있으면 같이 연결한다.
...
- PDF를 새로 만들거나 파일명이 바뀌면 웹앱의 다운로드 링크도 함께 확인한다.
