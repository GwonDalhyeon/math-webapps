...
- 점, 직선, 평면 등의 이름 라벨은 도형의 3D 기준점(anchor)에 연결하되 도형 위에 직접 겹치지 않도록 배치한다. 카메라의 시점 방향에 따라 화면의 오른쪽·위쪽 등 도형 바깥 방향으로 여백을 보정하고, 회전·확대·축소 중에도 도형과 연결된 상태를 유지한다.
- 장면 전환에서 라벨의 기준점과 화면 표시 위치를 분리한다. 전환 대상(`transitionTarget`)은 원래 도형 기준점을 유지하고, 화면 여백만 카메라 기준으로 매 프레임 계산한다.
...
- 정면·사선·위 시점과 확대·축소에서 라벨이 도형의 실루엣을 가리지 않는가?
- 카메라를 회전해도 라벨이 도형 주변의 읽기 쉬운 위치에 유지되는가?
- 장면·사례 전환 후에도 라벨의 기준점과 위치 보정이 정상적으로 동작하는가?
